...
public class FallingBlockPlacementServerSystem extends BaseComponentSystem {
    ...
    private static final String COMPLETE_STRUCTURE_ACTION_ID = "StructureTemplates:FallingBlockPlacementServerSystem" +
            ":completeStructure";
    private static final String PLACE_BLOCK_ACTION_ID = "StructureTemplates:FallingBlockPlacementServerSystem" +
            ":placeBlock";
    private static final String DESTROY_ENTITY_ACTION_ID = "StructureTemplates:FallingBlockPlacementServerSystem" +
            ":destroyEntity";
    private static final String CREATE_ENTITY_ACTION_ID = "StructureTemplates:FallingBlockPlacementServerSystem" +
            ":createEntity";
    private static final String COMPLETE_CREATE_ENTITY_ACTION_ID = "StructureTemplates" +
            ":FallingBlockPlacementServerSystem:completeCreateEntity";
    ...
    /**
     * This overrides the normal instant structure template spawning with one where an animation of falling blocks gets
     * played. The event {@link StructureSpawnStartedEvent} still gets sent at the start and the event {@link
     * StructureBlocksSpawnedEvent} gets send when the fall down animation has been played for all blocks. So there
     * might be a few seconds between the events.
     */
    ...
    public void onSpawnStructureEventWithBlocksPriority(SpawnStructureEvent event, EntityRef entity) {
        ...
        GetStructureTemplateBlocksForMidAirEvent getBlocksEvent =
                new GetStructureTemplateBlocksForMidAirEvent(event.getTransformation());
    ...
    void createFallingBlockEntities(Map<Vector3i, Block> blocksToPlace, EntityRef entityRef) {
        ...
        for (Map.Entry<Vector3i, Block> entry : blocksToPlace.entrySet()) {
            if (!entry.getValue().getURI().equals(BlockManager.AIR_ID)) {
                ...
                PrepareFallingBlockEntityComponent prepareBlockEntityComponent =
                        new PrepareFallingBlockEntityComponent();
    ...
    private void createEntityThatTriggersTheCompletionEvent(EntityRef structureTemplate,
                                                            BlockRegionTransform transform) {
    ...
    public void onFallingBlockDelayElapsed(DelayedActionTriggeredEvent event, EntityRef entityRef,
        ...
        if (event.getActionId().equals(PLACE_BLOCK_ACTION_ID)) {
            ...
            if (block == null) {
                logger.error("Block with url not found, block placement of fallen block skipped: " + component.blockUri);
            }
