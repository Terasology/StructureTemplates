...
/**
 * This event gets sent to the structure template editor entity (the origin block) when a new structure template should
 * be created from blocks that have been marked by that structure template editor.
 */
public class BuildStructureTemplateEntityEvent implements Event {
    ...
    public Collection<Vector3i> findAbsolutePositionsOf(BlockFamily blockFamily) {
        ...
        for (Block block : blockFamily.getBlocks()) {
            Collection<Vector3i> positionsOfBlock = blockToAbsolutePositionMap.getOrDefault(block,
